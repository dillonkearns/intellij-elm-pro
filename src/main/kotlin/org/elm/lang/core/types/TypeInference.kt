...
private class InferenceScope(
    ...
    private fun beginLambdaInference(lambda: ElmAnonymousFunctionExpr): InferenceResult {
        ...
        return InferenceResult(expressionTypes, diagnostics, recordDiffs, TyFunction(paramVars, bodyTy).uncurry(), pipelineTypes)
    }
    ...
    private fun beginLetInInference(letIn: ElmLetInExpr): InferenceResult {
        ...
        return InferenceResult(expressionTypes, diagnostics, recordDiffs, exprTy, pipelineTypes)
    }
    ...
    private fun beginCaseBranchInference(
        ...
        return InferenceResult(expressionTypes, diagnostics, recordDiffs, ty, pipelineTypes)
    }
    ...
    private fun toTopLevelResult(ty: Ty, replaceExpressionTypes: Boolean = true): InferenceResult {
        ...
        return InferenceResult(exprs, diagnostics, recordDiffs, ret, pipelineTypes)
    }
        ...
        fun validateTree(tree: BinaryExprTree<ElmBinOpPartTag>): TyAndRange {
            return when (tree) {
                is BinaryExprTree.Operand -> {
                    if (tree.operand is ElmOperandTag) {
                        val ty = inferOperand(tree.operand)
                        TyAndRange(tree.operand, ty)
                    } else {
                        TyAndRange(tree.operand, TyUnknown())
                    }
                }
